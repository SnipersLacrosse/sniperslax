...
$_SESSION['WPOA']['PROVIDER'] = 'SPORTiD';
define('HTTP_UTIL', get_option('wpoa_http_util'));
define('CLIENT_ENABLED', get_option('wpoa_sportid_api_enabled'));
define('CLIENT_ID', get_option('wpoa_sportid_api_id'));
define('CLIENT_SECRET', get_option('wpoa_sportid_api_secret'));
define('REDIRECT_URI', rtrim(site_url(), '/') . '/');
define('SCOPE', 'user'); // PROVIDER SPECIFIC: "user" is the minimum scope required to get the user's id from SPORT iD

if (strpos($_SERVER['HTTP_HOST'], 'localhost') !== false) {
	define('URL_AUTH', "http://localhost:3000/oauth/authorize?");
	define('URL_TOKEN', "http://localhost:3000/oauth/token?");
	define('URL_USER', "http://localhost:3000/oauth/me?");
} else {
	define('URL_AUTH', "https://api.sportidapp.com/oauth/authorize?");
	define('URL_TOKEN', "https://api.sportidapp.com/oauth/token?");
	define('URL_USER', "https://api.sportidapp.com/oauth/me?");
}
...
elseif (isset($_GET['code'])) {
	...
	if ($_SESSION['WPOA']['STATE'] == $_GET['state']) {
		...
		get_oauth_token($this); // if (!get_oauth_token($this)) { login?! }
